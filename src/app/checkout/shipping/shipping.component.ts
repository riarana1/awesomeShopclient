...
export class ShippingComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.shippingForm = new FormGroup({
      shipAddress: new FormControl(null, [
        Validators.pattern('[0-9a-zA-Z #,-]+'),
        Validators.required,
        BlankValidators.notBlankValidator,
  ...
  onSubmitOrderForm() {
    const postData = {
      ...
      billingAddress: this.billingAddressEnabled
        ? this.shippingForm?.value.billingAddress.trim()
        : null,
    ...
    this.store.dispatch(OrderActions.postShipping({ shipping: postData }));
