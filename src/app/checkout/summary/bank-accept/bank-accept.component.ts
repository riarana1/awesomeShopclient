...
export class BankAcceptComponent implements OnInit {
  ...
  postOrder: Checkout | undefined;
  postCart: Cart | undefined;
    ...
    this.orderState.subscribe((orderState: OrderState) => {
      this.postOrder = {
        ...
        billingAddress: orderState.shipping!.billingAddress
          ? orderState.shipping!.billingAddress
          : orderState.shipping!.shipAddress,
  ...
  paymentConfirm() {
    this.cartService
      ...
      .subscribe(() => {
        this.store.dispatch(
          OrderActions.postOrder({ checkout: this.postOrder! })
        );
      });
