...
export class CheckoutGuardService
  implements CanActivate, CanActivateChild, CanDeactivate<CheckoutComponent>
{
    ...
    return this.store.select('cart').pipe(
      ...
      map(
        (cartState: fromCart.CartState) =>
          cartState.cart && cartState.cart.cartItems.length !== 0
      )
    );
  }
  ...
  canDeactivate(
    ...
    this.store.dispatch(OrderActions.isCheckoutActive({ active: false }));
    return true;
  }
}
