...
export class CartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.cartItemCountSubscription = this.cartState.subscribe((data) => {
      if (data.cart && data.cart.cartItems.length) {
        this.cartItemCount = data.cart.cartItems.reduce(
  ...
  applyCode(discountCodeField: HTMLInputElement) {
    ...
    this.store.dispatch(CartActions.applyDiscount({ code: discountCode }));
  }

  goToItem(productUrl: String) {
  ...
  amountIncrement(id: number) {
    this.store.dispatch(CartActions.incrementCart({ id, amount: '1' }));
  }

  amountDecrement(id: number) {
    this.store.dispatch(CartActions.decrementCart({ id, amount: '1' }));
  }
  ...
  activatePurchase() {
    this.store
      ...
      .subscribe((data) => {
        if (data.isActive) {
          this.store.dispatch(OrderActions.isCheckoutActive({ active: true }));
          this.router.navigate(['/checkout/personal'], {
            relativeTo: this.route,
          });
        } else {
          alert(
            'Your account is inactive. You must activate your account in order to purchase.\nPlease check your email.'
          );
        }
      });
  }
