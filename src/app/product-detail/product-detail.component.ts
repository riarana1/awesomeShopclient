...
export class ProductDetailComponent implements OnInit, OnDestroy {
  ...
  activeProductVariant: ProductVariantDetails | undefined;
  ...
  isPopState = false;
  fetchError: HttpErrorResponse = null as any;
  ...
  ngOnInit() {
    ...
    this.paramSubscription = this.route.params.subscribe((params: Params) => {
      ...
      this.productService
        .getFullProduct(this.productUrl)
        .pipe(
          take(1),
          catchError((error) => {
        ...
        .subscribe((data: ProductDetail) => {
          ...
          this.variant = params['variant']
            ? params['variant']
            : this.product.productVariantDetails[0].id;
          this.activeProductVariant = data.productVariantDetails.find(
            (p) => p.id === Number(this.variant)
          );
        ...
        if (data.authenticated) {
          this.store.dispatch(
            CartActions.addToCart({
              id: this.activeProductVariant!.id,
              amount,
            })
          );
        } else {
